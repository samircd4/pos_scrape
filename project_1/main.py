...
def save_data(data, filename ='orders.xlsx'):
    try:
...
def read_data(filename):
    ...
    for index, row in df.iterrows():
        ...
        if str(order_id) in oerders:
            continue
        oerders.append(str(row['order_id']))
    return oerders

def get_orders(day, month, year, order_type, filename='orders.xlsx'):
    url = 'https://wdfpos.com/reports'
    console.log('Navigated to', url)
    orders = []
    existing_orders = read_data(filename)
    with sync_playwright() as p:
        ...
        for i in items:
            ...
            try:
                ...
                if str(order_id) in existing_orders:
            ...
            try:
                if button:
                    ...
                    for p in products_len:
                        ...
                        table.add_row(created_date,order_id,order_type,quantity_details, unit_price, pay_status, subtotal)
                        # print(order)
            except Exception as e:
                console.log(f'{order_id} Order id is not scraped due to {e}')
                with open('error.txt', 'a') as f:
                    f.write(f'{order_id}  -> Date:  {int(month)+1}-{day}-{year} <- ->{order_type}<- ERROR\n')
                # save_data(orders, filename)
                continue
    ...
    save_data(orders, filename)
    return orders
def recheck(filename='orders.xlsx', order_id='2536'):
    url = 'https://wdfpos.com/reports'
    console.log('Navigated to', url)
    orders = []
    existing_orders = read_data(filename)
    with sync_playwright() as p:
        ...
        page.goto(url)
        page.get_by_role("combobox").first.select_option('\"Order List\"')
        date_input = page.query_selector_all('div.date_picker_element')
        date_input[0].click()
        # page.query_selector_all('div.date_picker_element')[0].click()
        page.select_option("select.picker__select--year", value="2023")
        page.select_option("select.picker__select--month", value="6")
        page.click(f'div.picker__day.picker__day--infocus:has-text("1")')
        console.log('Start date selected')
        ...
        page.locator('input.cmaSjaC').fill(order_id)
        page.get_by_role("button", name="Display Report").click()
...
""")
    ...
    get_orders(day, month, year, order_type, filename=filename)
